...
def _wait_for_execution(execution_id, headers):
    ...
    while True:
        ...
        if execution_status == 'terminated':
            ...
            raise RuntimeError('Execution with id {0} failed'.
                               format(execution_id))
        sleep(poll_interval)
...
def create_upgrade_snapshot():
    ...
    url = 'http://localhost/api/v2/snapshots/{0}'.format(snapshot_id)
    ...
    if res.code != 201:
        raise RuntimeError('Failed creating snapshot {0}'
                           .format(snapshot_id))
...
def restore_upgrade_snapshot():
    ...
    url = 'http://localhost/api/v2/snapshots/{0}/restore'.format(snapshot_id)
    ...
    if res.code != 200:
        raise RuntimeError('Failed restoring snapshot {0}'
                           .format(snapshot_id))
    execution_id = json.loads(res.readlines()[0])['id']
    _wait_for_execution(execution_id, headers)
...
def _generate_upgrade_snapshot_id():
    ...
    if res.code != 200:
        ctx.logger.error('Failed extracting current manager version.')
